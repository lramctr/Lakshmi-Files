...
PivotViewer.Views.GraphView = PivotViewer.Views.TileBasedView.subClass({
	...
	GetButtonImage: function () {
		return '/NpsGis/images/GraphView.png';
	},
	GetButtonImageSelected: function () {
		return '/NpsGis/images/GraphViewSelected.png';
	},
...
PivotViewer.Views.GridView = PivotViewer.Views.TileBasedView.subClass({
	...
	GetButtonImage: function () {
		return '/NpsGis/images/GridView.png';
	},
	GetButtonImageSelected: function () {
		return '/NpsGis/images/GridViewSelected.png';
	},
	GetViewName: function () {
		return 'Grid View';
	},
...
PivotViewer.Views.MapView = PivotViewer.Views.IPivotViewerView.subClass({
	...
	GetUI: function () { return ''; },
	GetButtonImage: function () {
		return '/NpsGis/images/MapView.png';
	},
	GetButtonImageSelected: function () {
		return '/NpsGis/images/MapViewSelected.png';
	},
...
PivotViewer.Views.TableView = PivotViewer.Views.IPivotViewerView.subClass({
	...
	Setup: function (width, height, offsetX, offsetY, tileMaxRatio) {
		...
		$('.pv-viewpanel').append("<div style='visibility:hidden;position:relative;' id='pv-table-loader'><img src='/NpsGis/images/loading.gif'></img></div>");
	...
	GetButtonImage: function () {
		return '/NpsGis/images/TableView.png';
	},
	GetButtonImageSelected: function () {
		return '/NpsGis/images/TableViewSelected.png';
	},
	GetViewName: function () {
		return 'Table View';
	},
						   ...
						   for (l = 0; l < this.tiles[j].facetItem.Facets[k].FacetValues.length; l++) {
							  ...
							  if (this.IsFilterVisible (attribute)) {
								  ...
								  if (this.tiles[j].facetItem.Href) {
									  // Value is uri
									  if (this.IsUri(value))
										  tableRows.push({key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven +"'><td id='pv-key' class='tooltipinter' title='Toggle item selection' style='color:#009933;cursor:pointer'>" + entity + " " + "<a href=" + this.tiles[j].facetItem.Href.replace(/'/g, "%27") + " target=\"_blank\"><img style='cursor:default;' id='pv-linkout' title='Follow the link' src='/NpsGis/images/goout.gif'></img></a></td><td id='pv-facet' class='tooltipcustom'  title='Show only this relation' style='color:#009933;cursor:pointer'>" + attribute + "</td><td id='pv-value' class='tooltipinter' title='Filter on this value' style='color:#009933;cursor:pointer'>" + value + " " + "<a href=" + value + " target=\"_blank\"><img style='cursor:default;' id=pv-linkout' title='Follow the link' src='/NpsGis/images/goout.gif'></img></a></td></tr>"});
									  else
...
(function ($) {
	...
	InitPreloader = function () {
		//http://gifmake.com/
		_self.append("<div class='pv-loading'><img src='/NpsGis/images/loading.gif' alt='Loading' /><span>Loading...</span></div>");
	...
	InitUI = function () {
		...
		toolbarPanel += "<h5 class='pv-toolbarpanel-name'>" + PivotCollection.CollectionName + "</h5>";
	...
	CreateFacetList = function () {
		...
		for (var i = 0; i < PivotCollection.FacetCategories.length; i++) {
			if (PivotCollection.FacetCategories[i].IsFilterVisible) {
				facets[i + 1] = "<h6><a href='#' title=" + PivotCollection.FacetCategories[i].Name + ">";
				facets[i + 1] += PivotCollection.FacetCategories[i].Name;
				facets[i + 1] += "</a><div class='pv-filterpanel-accordion-heading-clear' facetType='" + PivotCollection.FacetCategories[i].Type + "'>&nbsp;</div></h6>";
